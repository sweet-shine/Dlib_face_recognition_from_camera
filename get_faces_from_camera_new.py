...
class Face_Register:
    def __init__(self):
        # 获取照片存储目录
        self.path_photos_from_camera = constants.path_photos_from_camera

        # 字体
        self.font = cv2.FONT_ITALIC

        self.existing_faces_cnt = 0  # 已录入人脸的人数计数器
        self.ss_cnt = 0  # 录入 personX 人脸时图片计数器
        self.faces_cnt = 0  # 录入人脸计数器

        # 之后用来控制是否保存图像的 flag / The flag to control if save
        self.save_flag = 1
        # 之后用来检查是否先按 'n' 再按 's' 。只有先按n再按s/ The flag to check if press 'n' before 's'
        self.press_n_flag = 0
    ...
    # 每次采集新的人脸，都会删除之前存的人脸数据文件夹 / Delete the old static of faces
    ...
    def check_existing_faces_cnt(self):
        if os.listdir(constants.path_photos_from_camera):
            # 获取已录入的最后一个人脸序号 / Get the num of latest person
            person_list = os.listdir(constants.path_photos_from_camera)
            person_num_list = []
    ...
    def update_fps(self):
        ...
        # print(self.frame_time,self.frame_start_time)
        self.fps = 1.0 / self.frame_time
        # print(self.fps)
        self.frame_start_time = now
        # print(self.frame_start_time)
...
@app.route('/get_faces_from_camera', methods=['POST'])
def get_faces_from_camera():
